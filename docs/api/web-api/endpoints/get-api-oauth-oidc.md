...
handler: controller.OidcAuth
source: router/api-router.go:46
request:
  query_params:
    - state
    - code
...
OIDC OAuth 回调。未登录时用于登录/注册；已有登录 session 时转为绑定 OIDC 账号。

## 查询参数字段

- `state`: 字符串，必填。必须等于 session 中保存的 OAuth state。
- `code`: 字符串，必填。OIDC 授权码。
...
- HTTP 200 且 `success=false`: 可能为 `管理员未开启通过 OIDC 登录以及注册`、`用户已注销`、`管理员关闭了新用户注册`、用户被禁用提示，或 OIDC API 错误。
