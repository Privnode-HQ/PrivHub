...
func TestUserGroupUsageLimitsRoundTripSupportsHourlyAndDaily(t *testing.T) {
	...
	input := `{"default":{"rpm":5,"rpd":20,"tpm":2000,"tpd":10000,"hourly":300,"daily":900,"monthly":3000}}`
	...
	var serializedPolicies map[string]map[string]int64
	...
	if groupPolicy["rpm"] != 5 || groupPolicy["rpd"] != 20 || groupPolicy["tpm"] != 2000 || groupPolicy["tpd"] != 10000 {
		t.Fatalf("unexpected serialized request/token limits: %#v", groupPolicy)
	}
	if groupPolicy["hourly"] != 300 || groupPolicy["daily"] != 900 || groupPolicy["monthly"] != 3000 {
