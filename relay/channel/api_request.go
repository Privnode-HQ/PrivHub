...
// processHeaderOverride 处理请求头覆盖，支持变量替换和请求头透传。
// 支持的变量：
//   - {api_key}: 替换为渠道 API Key
//   - {client_header:<name>}: 替换为客户端请求头值
//
// 透传规则写在 key 上，value 会被忽略：
//   - "*": 透传所有安全的客户端请求头
//   - "re:<regex>" / "regex:<regex>": 透传匹配正则的请求头名
//
// 透传先执行，显式 Header Override 后执行，因此显式配置优先级最高。
