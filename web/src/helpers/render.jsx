...
export const DEFAULT_QUOTA_PER_UNIT = 1000000;

export function normalizeQuotaDisplayType(value) {
  const normalized = String(value || '')
    .trim()
    .toUpperCase();
  return normalized === 'CNY' ? 'CNY' : 'USD';
}

export function getQuotaDisplayType() {
  return normalizeQuotaDisplayType(localStorage.getItem('quota_display_type'));
}

export function getQuotaCurrencySymbol() {
  return getQuotaDisplayType() === 'CNY' ? '¥' : '$';
}
...
export function renderQuotaNumberWithDigit(num, digits = 2) {
  ...
  return getQuotaCurrencySymbol() + num.toFixed(digits);
}
...
export function renderUnitWithQuota(quota) {
  return getQuotaPerUnit() * parseFloat(quota || 0);
}
...
export function renderQuotaWithAmount(amount) {
  return getQuotaCurrencySymbol() + amount;
}
...
export function quotaToCurrencyAmount(quota) {
  ...
  return numericQuota / getQuotaPerUnit();
}

export function currencyAmountToQuota(amount) {
  ...
  return Math.round(numericAmount * getQuotaPerUnit());
}
...
export function getCurrencyConfig() {
  const type = getQuotaDisplayType();
  return { symbol: getQuotaCurrencySymbol(), rate: 1, type };
}
...
export function convertUSDToCurrency(usdAmount, digits = 2) {
  ...
  return symbol + Number(usdAmount || 0).toFixed(digits);
}

export function renderQuota(quota, digits = 2) {
  const symbol = getQuotaCurrencySymbol();
